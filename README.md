...
pnpm 在云端环境由于代理问题可能存在延迟缓慢，影响项目初始化速度。

**解决方案**：考虑使用 pnpm 镜像源或预构建依赖缓存。
...
云端部署 localhost 无法跳转，需要使用云公共 IP，存在安全隐患与使用体验问题。

**临时方案**：已在 PreviewFrame 组件中添加云端 IP 配置功能。
